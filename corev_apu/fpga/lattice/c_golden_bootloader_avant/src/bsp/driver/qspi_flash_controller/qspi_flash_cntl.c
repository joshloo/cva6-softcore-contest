...
void enable_xip_mode(unsigned int mode, unsigned int flash_addr, unsigned int opCode) {
	 ...
	 * with 8’hEB opcode to enable the XiP mode of the Macronix flash for the succeeding reads
	 */
	if (mode == ENABLE) {
		...
		/* Disabling the XiP mode is same with enabling steps with xfer_len_bytes changed to 16’h6
		 * since command will not be sent during XiP enabled access and the
		 * 2-byte pattern will change to 16’hAA55 or 16’hFF00
		 */
